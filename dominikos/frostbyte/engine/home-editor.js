...
export const SNAP = 4 * 3; // world px — native 4px grid at the default scale

// Snap a single world coordinate to the SNAP grid.
function snapValue(v, scale = 3) {
  const snap = 4 * scale;
...
// Snap then clamp so the item's scaled WORLD half-extents stay inside `bounds`.
function snapAndClamp(x, y, item, bounds, scale = 3) {
  const hw = (item.w * scale) / 2;
  const hh = (item.h * scale) / 2;
  return {
    x: clampValue(snapValue(x, scale), bounds.x0 + hw, bounds.x1 - hw),
    y: clampValue(snapValue(y, scale), bounds.y0 + hh, bounds.y1 - hh),
  };
}
...
export function place(home, furniture, item, x, y, bounds, maxPlaced, ev = [], scale = 3) {
  ...
  const { x: px, y: py } = snapAndClamp(x, y, item, bounds, scale);
  furniture[item.id] -= 1;
...
export function move(home, index, x, y, item, bounds, ev = [], scale = 3) {
...
// Topmost placed index whose world rect (center +/- scaled half-extents from catalogById[entry.id].w/h)
// contains (x,y); -1 on miss. Iterates from the END of `placed` so later (on-top) entries win ties.
export function hitTest(home, catalogById, x, y, scale = 3) {
