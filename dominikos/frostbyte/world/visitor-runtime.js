...
function pickTarget(getPlaced, bf) {
  ...
  const x = Math.min(TARGET_X_MAX, Math.max(TARGET_X_MIN, item.x + Math.cos(angle) * FURNITURE_OFFSET * bf));
  const y = Math.min(TARGET_Y_MAX, Math.max(MIN_TARGET_Y, item.y + Math.sin(angle) * FURNITURE_OFFSET * bf));
  return { x, y };
}
...
function showBubble(k, root, text, bf) {
  if (!text) return null;
...
export function initVisitorLayer(k, { scale, doorPos, getPlaced }) {
  ...
  function handle(event) {
    ...
    switch (event.type) {
      case 'visit-start': {
        ...
        actor.root.z = pos.y;
        target = pickTarget(getPlaced, bf);
        break;
      }
      case 'visit-line': {
        if (!actor) break;
        ...
        bubble = showBubble(k, actor.root, line.text, bf);
        if (bubble) {
          const mine = bubble;
  ...
  function tick(dt) {
    ...
    if (target) {
      ...
      if (dist > ARRIVE_EPS) {
        const speed = SPEED_PX_PER_S * Math.sqrt(bf) * (persona?.speedMul ?? 1);
