...
1. **Wallpaper:** original hill SVG (recommended, zero-risk) vs a verified CC0 photo? (Default: SVG.)
2. **Start button label:** keep "start" (parody) or use "DM"/"Menu" to reduce trade-dress similarity?
3. **Make `/os/` the root** later via `/ → /os/` redirect (classic at `/classic/`), or keep `/` as the front door indefinitely?
4. **Web3Forms access key** — provide it (public-safe) or ship mailto-only for v1?
5. **Résumé source of truth** — author the HTML fallback content (Fable generates PDF from it) sign-off.
6. **game2** timing — build now (its own Vite `base:'/game2/'` + bridge) or defer to P7?
7. **Vista/Aero** — ever want it shipped, or is XP-only fine permanently?
8. **PWA install** — do you want DominikOS installable to home screen (adds Workbox + icons)?
